...
- `input-method/rime/`：Rime/Squirrel 方案、皮肤、Lua 与小型辅助词典。
...
- Rime 的安装 ID、用户词频数据库、同步目录、编译产物和 `custom_phrase.txt` 个人短语。
...
Rime 使用 [iDvel/rime-ice](https://github.com/iDvel/rime-ice) 作为上游主词库。新机器应先安装 Squirrel 与 Rime Ice，再运行 `./install.sh` 覆盖个人方案，最后在输入法菜单中选择“重新部署”。主词库不重复放进本仓库。
